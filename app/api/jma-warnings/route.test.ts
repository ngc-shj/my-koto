...
function makeRequest(
  method: string,
  url = "https://my-koto.example.com/api/jma-warnings",
...
// Normalized output shape stored in KV
const staleWarningData = {
  reportDatetime: "2026-06-01T10:00:00+09:00",
  areaCode: "1310800",
  warnings: [{ code: 3, status: "発表" }],
};
...
describe("GET /api/jma-warnings", () => {
  ...
  it("returns Cache-Control with s-maxage and stale-if-error on 200", async () => {
    global.fetch = vi.fn().mockResolvedValue(
      new Response(JSON.stringify(validWarningPayload), {
        status: 200,
    ...
    expect(cc).toContain(`s-maxage=${JMA_WARNING_CACHE.SHARED_MAX_AGE}`);
    expect(cc).toContain(`stale-if-error=${JMA_WARNING_CACHE.STALE_IF_ERROR}`);
    expect(cc).toContain(`max-age=${JMA_WARNING_CACHE.BROWSER_MAX_AGE}`);
    expect(cc).toContain(`stale-while-revalidate=${JMA_WARNING_CACHE.STALE_WHILE_REVALIDATE}`);
  });
  ...
  it("returns stale KV data when upstream fails (stale-if-error)", async () => {
    ...
    const staleKey = "jma-warn:v1:130000:1310800";
    await (kv.kv.set as ReturnType<typeof vi.fn>)(staleKey, staleWarningData);
    ...
    expect(body.reportDatetime).toBe("2026-06-01T10:00:00+09:00");
    expect(body.areaCode).toBe("1310800");
