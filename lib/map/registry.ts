...
export const LAYERS: readonly LayerConfig[] = [
  ...
  },
  {
    ...
    letter: "B",
    // The Koto-bbox bundle (data/bus-toei.json, served via
    // /api/map/bus + IndexedDB) is authoritative inside the ward;
    // outside it we fall back to OSM highway=bus_stop nodes so the
    // same layer keeps working when users pan into 中央区 etc.
    osmTags: [
